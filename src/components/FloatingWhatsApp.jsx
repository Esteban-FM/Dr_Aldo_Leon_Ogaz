...
/**
 * Botón flotante fijo de WhatsApp (estilo Clínica Cuautitlán).
 * Recibe un mensaje distinto según la página donde se use, para que
 * el chat llegue pre-cargado con contexto (ej. tipo de consulta).
 */
export default function FloatingWhatsApp({ mensaje = "Hola, quisiera agendar una consulta." }) {
  ...
  return (
    <a
      ...
      className="fixed bottom-4 right-4 md:bottom-6 md:right-6 z-50 flex items-center gap-2 rounded-full bg-emerald-500 px-5 py-3 text-white shadow-lg shadow-emerald-500/30 transition-transform hover:scale-105"
      aria-label="Agendar cita por WhatsApp"
    >
      {/* Ícono WhatsApp simplificado en SVG (sin dependencias externas) */}
